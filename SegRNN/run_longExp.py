...
parser.add_argument('--model', type=str, required=True, default='Autoformer',
                    help='model name, options: [Autoformer, Informer, Transformer, Retnet]')
...
# 假设args是你的参数配置，且已经正确设置
exp_main = Exp_Main(args)

# 加载整个模型
exp_main.model = torch.load('/home/willy/research/apartment/checkpoints/Electricity_96_96_SegRNN_custom_ftM_sl96_pl96_dm512_dr0.1_rtgru_dwpmf_sl48_test_0_ausdata/complete_model.pt')
exp_main.model.to(exp_main.device)  # Make sure the model is on the correct device
exp_main.model.eval()


# Load test data
_, test_loader = exp_main._get_data(flag='test')

# exp_main.lime_explain(test_loader, instance_index=0)


# # Debugging and SHAP explanation setup
# exp_main.model.train()

# sample_data = next(iter(test_loader))[0]
# print("Sample data shape:", sample_data.shape)

# test_data = next(iter(test_loader))[0].float().to(exp_main.device)
...
# output = exp_main.model(test_data)
# output.sum().backward(retain_graph=True)  # Retain graph for further backward calls

# print("test_data.requires_grad:", test_data.requires_grad)  # Check if gradient tracking is enabled

# exp_main.model.eval()  # Set model to evaluation mode before SHAP explanation

# print(f"Model training mode before SHAP call: {exp_main.model.training}")  # Check model mode

# # Execute SHAP explanation
# exp_main.shap_explain(test_loader, output_index=0)

# exp_main.model.eval()  # Ensure the model is back in evaluation mode after SHAP explanation